...
class Inpainting_GAN(Inpainting):
    ...
    def init_model(self):
        ...
        input_layer = lasagne.layers.InputLayer((None, 64, 64, 3), self.y)
        layer = lasagne.layers.dimshuffle(input_layer, [0,3,1,2])
        self.disciminator1 = self.Discriminate(layer)
        self.disciminator2 = self.Discriminate(layer)
        self.disciminator = lasagne.layers.ElemwiseMergeLayer([self.disciminator1, self.disciminator2],
                                                              lambda x,y: (x+y)/2)
...
#                     cost = self.f_update(x, y ,dataset.mask)
                    ...
                    if numpy.mod(uidx, validFreq) == 0:
                        ...
                        self.show_examples(dataset, batch_size, 'figures/HGAN_epoch' + str(eidx) + '.png')
